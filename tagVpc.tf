locals {
  ...
  flattenIntranetSubnetV2 = var.BindTagIntranetSubnet == true ? flatten([
    for subnetID in toset(var.INTRANET_SUBNET_IDS) : [
      for tagdetail in data.google_tags_tag_value.selected_values : {
        subnetID  = subnetID
        tag       = tagdetail.namespaced_name
        parentID  = "//compute.googleapis.com/projects/${var.PROJECT_NUM}/regions/asia-southeast1/subnetworks/${subnetID}"
        tag_value =   tagdetail.id
      }
    ]
  ]) : []
...
resource "google_tags_location_tag_binding" "IntranetBinding" {
  ...
  for_each = {
    for tags in local.flattenIntranetSubnetV2 : "${tags.tag}-${tags.subnetID}" => tags if var.BindTagIntranetSubnet
  }
